...
```{r}
# cannot find annotation data through geoQuery so had to download
# retrieve GEO data set and clean data
...
for(disease in unique(phenoData$`disease:ch1`)){
  for(treatment in names(trt)){
    ...
    # remove genes that are not in eset_subset
    dbs <- lapply(dbs_all, function(db){
      ligands_to_match <- unique(c(treatment, rownames(eset_subset)))
      x = lapply(db, function(ligand){
        ligand[ligand %in% ligands_to_match]
      })
      
      db <- db[names(db) %in% ligands_to_match]
...
results_path <- here::here("results",
                   paste0(geo_data,"updatedRankList",".rds"))
saveRDS(do.call(rbind, rank_list), results_path)
