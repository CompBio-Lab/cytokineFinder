...
cytosig_results_long <- cytosig_results %>% 
  ...
  mutate(rank = 100*(rank(abs(z_scores))/n())) %>%
  group_by(ID) %>%
  arrange(rank)

cytosig_trt <- cytosig_results_long %>%
  filter(enriched_signals %in% c("TNFA", "IL1B","IL6")) %>%
  mutate(method = "cytosig", 
         rank = round(rank),
         method_db = "cytosig",
         database = "cytosig",
         enriched_signals = gsub("IL1B", "IL-1b", enriched_signals)) %>%
  mutate(enriched_signals = gsub("IL6", "IL-6", enriched_signals)) %>%
  mutate(enriched_signals = gsub("TNFA", "TNF", enriched_signals)) %>%
  separate(ID, into = c("disease", "cytokine", "logFC"), sep = "_") %>%
  filter(cytokine == enriched_signals)

```

```{r}
gse215039 %>% 
  group_by(method_db, cytokine, disease, rank) %>% 
  summarise(mean = mean(rank, na.rm = FALSE),  
            sd = sd(rank, na.rm = FALSE),  # Conditional SD
            db = unique(database),
            method = unique(method)) %>%  
  ggplot(aes(x = reorder(db, mean), y = mean, fill = method)) +
...
```
# Merge cytosig data together with other benchmarking methods
```{r}
subset_gse215039 <- gse215039 %>%
  select(method, rank, cytokine, disease, database, method_db)
subset_cytosig <-cytosig_trt %>%
  select(method, rank, cytokine, disease) %>%
  mutate(database = "cytosig",
         method_db = "cytosig")

all <- rbind(subset_gse215039, subset_cytosig) %>%
  group_by(method_db, database, cytokine, disease, rank) %>% 
  summarise(mean = mean(rank, na.rm = FALSE),  
            sd = sd(rank, na.rm = FALSE),  # Conditional SD
            db = unique(database),
            method = unique(method)) %>%
...
all %>%  
  ggplot(aes(x = reorder(method_db, mean), y = mean, fill = method)) +
  geom_bar(stat = "identity", position = "dodge") + 
#  geom_errorbar(aes(ymin=mean-1, ymax=mean+sd), width=.2,
#             position=position_dodge(.9)) +
  ylab("Mean Rank By Study/Cytokine") +
